 ...
 *
 * Runs after this package's own library build, which `npm run build:design`
 * invokes first: `vite build` → dist/entry-design.js plus one dist/*.css.
 *
 * Both come from that single build, so the markup and the stylesheet agree on
 * every scoped class name by construction. `vite.config.ts` still pins
 * `generateScopedName` to match the apps, so a card looks like the real site.
 *
...
// The package's own library build, not an app's. `cssCodeSplit: false` puts
// every module's CSS into one file, so the first .css in dist/ is all of it.
const cssFile = (await readdir(dist).catch(() => [])).find((file) => file.endsWith('.css'))
if (!cssFile) {
  throw new Error(
    'design-bundle: no stylesheet in dist/ — run `npm run build -w @jonleibham/design-system` first',
  )
}

const appCss = await readFile(path.join(dist, cssFile), 'utf8')
const cardCss = await readFile(path.join(root, 'scripts', 'design-card.css'), 'utf8')
const css = `${appCss}\n${cardCss}`

const { cards, renderCard } = await import(
  new URL(`file://${path.join(dist, 'entry-design.js')}`).href
)
 ...
 * Against the package's own stylesheet this is a real check: it used to pass
 * largely because every component happened to be reachable from the app.
  ...
  if (missing.length > 0) {
    throw new Error(
      `design-bundle: card "${card.slug}" uses class(es) with no rule: ${missing.join(', ')}. ` +
        'A component whose CSS did not make the library build is not in dist/*.css.',
    )
  }
}
